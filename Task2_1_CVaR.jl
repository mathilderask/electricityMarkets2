...
@constraint(model, [i=1:N, m=1:T], c_up - F[i,m] <= xi[i,m])  # Constraint 1: c_up - F[i, m] ≤ xi[i, m]
@constraint(model, [i=1:N, m=1:T], (1 / (N * T)) * sum(xi[i, m]) <= (1 - epsilon) * beta) # Constraint 2: average of all xi[i,m] ≤ (1 - epsilon) * beta
@constraint(model, [i=1:N, m=1:T], beta <= xi[i, m]) # Constraint 3: beta ≤ xi[i, m] ∀ i, m
