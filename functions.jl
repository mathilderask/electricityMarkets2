...
function optimise_bidding_quantity3(p_real, lambda_DA, system_status, pricing_scheme) # later add the input: "pricing_scheme", where it will be either "one-price" or "two-price"
    ...
    opt_production = JuMP.value.(p)
    expected_profit = JuMP.objective_value(m)
    println("Expected profit in sample: ", expected_profit)
